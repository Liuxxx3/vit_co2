...
class ANETdetection(object):
    ...
    def __init__(
        ...
        self.ap = None
        self.annotation_path = os.path.join(annotation_path)
    ...
    def _import_ground_truth(self, annotation_path):
        ...
        info_test = np.load(annotation_path + "info_test.npy", allow_pickle=True)
        try:
        ...
        for i in range(len(gtsegments)):
            for j in range(len(gtsegments[i])):
                ...
                t_start_lst.append(round(gtsegments[i][j][0]*25/16*self.max_len/int(info_test[i])))
                t_end_lst.append(round(gtsegments[i][j][1]*25/16*self.max_len/int(info_test[i])))
